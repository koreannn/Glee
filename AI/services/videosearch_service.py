...
# 이후 절대 경로 임포트 사용
from AI.utils.get_embeddings import CustomEmbeddings
        ...
        try:
            ...
            if os.path.exists(dimension_file):
                ...
                if stored_dimension != current_dimension_size:
                    logger.info(f"임베딩 차원이 변경되었습니다 ({stored_dimension} -> {current_dimension_size}). 벡터 스토어를 재생성합니다.")
                    force_recreate = True
...
class VideoSearchService:
    ...
    async def get_or_fetch_relevant_content(self, situation:str, max_contents: int = 3, force_refresh: bool = False) -> dict:
        try:
            # 1st. 강제 옵션 확인
            ...
            if len(relevant_chunks) >= 3:
                logger.info(f"벡터 DB에 이미 컨텐츠가 충분히 많네요. 유튜브 검색 없이 종료합니다. (찾은 컨텐츠: {len(relevant_chunks)}개)")
    ...
    async def _search_and_update(self, situation:str, max_contents: int = 3) -> dict:
        try:
            ...
            youtube_results = self.youtube_search(situation, max_results=max_contents*5)  # 더 많은 결과를 가져와서 자막 있는 영상을 찾을 확률 높임
                ...
                try:
                    if item["id"]["kind"] == "youtube#video":
                        ...
                        if all([item["id"]["videoId"], item["snippet"]["title"], item["snippet"]["description"]]):
                            ...
                            logger.info(f"검색 결과: {{'id': '{item['id']['videoId']}', 'title': '{item['snippet']['title']}', 'description': '{item['snippet']['description'][:50]}...'}}")
                except Exception as e:
